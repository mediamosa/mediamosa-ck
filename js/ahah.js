...
Drupal.ahah = function(base, element_settings) {
  ...
  var options = {
    ...
    success: function(response, status) {
      ...
      // MADCAP patch.
      document.getElementById('edit-mediamosa-success').value = 'TRUE';
      return ahah.success(Drupal.parseJson('The file was successfully uploaded. Changes are not permanent until you save this post.'), status);
      // MADCAP patch - end.
      return ahah.success(response, status);
    },
    complete: function(response, status) {
      // MADCAP patch.
      if (document.getElementById('edit-mediamosa-file-upload').value != '') {
        document.getElementById('edit-mediamosa-success').value = 'TRUE';
        return ahah.success(Drupal.parseJson('The file was successfully uploaded. Changes are not permanent until you save this post.'), status);
      }
      else {
        if (status == 'error' || status == 'parsererror') {
          resp = new Object;
          resp.responseText = 'An error has occured';
          resp.status = 200;
          return ahah.error(resp, 'You have to choose a file first.');
        }
      }
      // MADCAP patch - end.
      if (status == 'error' || status == 'parsererror') {
